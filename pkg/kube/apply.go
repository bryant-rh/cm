...
func (o *applyOptions) Apply(ctx context.Context, data []byte, namespace string) error {
	...
	for _, unstruct := range unstructList {
		klog.V(4).Infof("Apply object: %#v", unstruct)
		if _, err := ApplyUnstructured(ctx, o.dynamicClient, restmapper, unstruct, namespace, o.serverSide); err != nil {
			return err
		}
		klog.V(4).Infof("%s/%s applyed", strings.ToLower(unstruct.GetKind()), unstruct.GetName())
	}
	return nil
}
...
func ObjectToUnstructured(obj runtime.Object) ([]unstructured.Unstructured, error) {
	...
	if meta.IsListType(obj) {
		...
		// 	list = append(list, u)
		// }
		list = append(list, obj.(*unstructured.UnstructuredList).Items...)
		return list, nil
	}
